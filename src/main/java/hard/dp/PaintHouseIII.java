...
public class PaintHouseIII {
    ...
    public int minCost(int[] houses, int[][] cost, int m, int n, int target) {
        //This will be the cost analysis if the houses were in neighborhood.
        int[][] prevCost = new int[target + 1][n];

        // Initialize prevCost array
                ...
                for (int color = 1; color <= n; color++) {
                    ...
                    for (int prevColor = 1; prevColor <= n; prevColor++) {
                        if (prevColor != color) {
                            ...
                            currCost = Math.min(currCost, prevCost[neighborhoods - 1][prevColor - 1]);
                        } else {
                            // Previous house has the same color, no change in neighborhood count
                            currCost = Math.min(currCost, prevCost[neighborhoods][color - 1]);
                        }
                    }
